...
urlpatterns = [
    ...
    path('index/', views.index, name='index'),  # Dashboard after login
    path('general-discussion/', views.general_discussion, name='general_discussion'),  # General Discussion page
    path('map/', views.map_view, name='map'),  # Child Abuse Cases Map
    path('support/', views.support, name='support'),  # Support Communities
    path('logout/', auth_views.LogoutView.as_view(next_page='logocover'), name='logout'),  # Logout
    ...
    # Report submission and dashboar
    path('send-report/', views.report_incident, name='report_incident'),  # POST endpoint for report submission
    path('dashboard/', views.user_dashboard, name='user_dashboard'),  # User dashboard view
